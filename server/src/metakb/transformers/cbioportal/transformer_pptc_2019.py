...
class cBioportalTransformer(Transformer):
    ...
    def transform(self, harvested_data):
        ...
        self.variants = self.variants.rename(
            columns={"User_Amino_Acid_Change": "Amino_Acid_Change"}
        )
        ...
        self.patients = self.patients.rename(
            columns={"INFERRED_ETHNICITY": "ETHNICITY"}
        )
        # Check duplicate count
        ...
        print(f"Number of duplicate rows : {num_duplicates}")
        # self.samples = self.samples.rename(columns={'GENE_PANEL': 'Sequence_Source'})
        # print duplicates (excluding first instance)
        if num_duplicates > 0:
            print("\nDuplicate rows (excluding first instance):")
