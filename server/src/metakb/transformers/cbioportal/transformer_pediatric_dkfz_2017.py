...
class cBioportalTransformer(Transformer):
    ...
    def transform(self, harvested_data):
        ...
        self.variants["Center"] = self.variants["Center"].replace(
            r"^\s*\.?\s*$", "DKFZ", regex=True
        )
        self.variants.loc[
            (self.variants["Hugo_Symbol"] == "RAB36")
            & (self.variants["Chromosome"] == "NA"),
            "Chromosome",
        ] = 22
        ...
        self.samples = self.samples.rename(
            columns={"ONCOTREE_CODE_CANCER_TYPE": "ONCOTREE_CODE"}
        )
        self.samples = self.samples.rename(
            columns={"SEQUENCING_TYPE": "Sequence_Source"}
        )
        # print duplicates (excluding first instance)
        if num_duplicates > 0:
            print("\nDuplicate rows (excluding first instance):")
