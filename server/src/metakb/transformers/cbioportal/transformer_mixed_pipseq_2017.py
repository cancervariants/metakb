...
class cBioportalTransformer(Transformer):
    ...
    def transform(self, harvested_data):
        ...
        self.variants["Center"] = self.variants["Center"].replace(
            r"^\s*\.?\s*$", "Columbia", regex=True
        )
        ...
        print(f"Number of duplicate rows : {num_duplicates}")
        # self.samples = self.samples.rename(columns={'ONCOTREE_CODE_CANCER_TYPE': 'ONCOTREE_CODE'})
        self.samples = self.samples.rename(columns={"NGS_TEST": "Sequence_Source"})
        # print duplicates (excluding first instance)
        if num_duplicates > 0:
            print("\nDuplicate rows (excluding first instance):")
        ...
        dupe_mask = combined_df.duplicated(
