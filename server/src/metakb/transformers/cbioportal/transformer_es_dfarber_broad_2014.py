...
class cBioportalTransformer(Transformer):
    ...
    def _correct_male_chrom23(self, df):
        ...
        if not ambig_rows.empty:
            print(
                f"⚠️ Found {len(ambig_rows)} row(s) with ambiguous chromosome identification."
            )
            print(
                ambig_rows[
    ...
    def _resolve_ambiguous_chromosomes(self, df):
        ...
        """
        chrom23_male_mask = (df["Chrom_23"] == True) & (
        ...
        for idx, row in df[chrom23_male_mask].iterrows():
            ...
            if x_id != "no_value" and y_id != "no_value":
                print(
                    f"⚠️ Row {idx}: Both x_hgnc_id and y_hgnc_id have values. Manual check recommended."
                )
            ...
            if ambig_status in ["XY", "neither"]:
                if x_id != "no_value" and y_id == "no_value":
                    ...
                    print(
                        f"⚠️ Index {idx}: Ambiguous Chromosome with both HGNC IDs present."
                    )
                else:
                    print(
                        f"❌ Index {idx}: Ambiguous Chromosome with no HGNC ID found."
                    )
        ...
        for idx, row in df[mask].iterrows():
            ...
            if chrom == "X":
                if gene_id == x_id and gene_id != "no_value":
                    ...
                    print(
                        f"❌ Row {idx}: X chromosome mismatch. gene_hgnc_id={gene_id}, x_hgnc_id={x_id}"
                    )
            elif chrom == "Y":
                if gene_id == y_id and gene_id != "no_value":
                    ...
                    df.at[idx, "hgnc_id_match"] = "no_match"
                    print(
                        f"❌ Row {idx}: Y chromosome mismatch. gene_hgnc_id={gene_id}, y_hgnc_id={y_id}"
                    )
        return df

    def transform(self, harvested_data):
        study = "es_dfarber_broad_2014"
        # Get current directory (you’re in: /Users/costellh/repos/metakb/server/src/metakb/harvesters)
        loc = os.getcwd()
        # Move one level up and into "transformers"
        base_dir = os.path.join(loc, "..", "transformers")
        # Normalize the path (resolves ".." to a clean absolute path)
        base_dir = os.path.abspath(base_dir)
        # Build and create the study directory
        study_out_dir = os.path.join(base_dir, "munged_data")
        save_loc = os.path.join(study_out_dir, study)
        os.makedirs(save_loc, exist_ok=True)

        # study_df_folder = os.path.join(save_loc, study, "FILE")
        ...
        if num_duplicates > 0:
            ...
            # assign age to NAs
            self.patients["AGE"] = (
                self.patients["AGE"].replace(r"^\s*$", pd.NA, regex=True).fillna("<21")
            )
            # remove duplicates, but keep first occurrence
        ...
        file_path = os.path.join(save_loc, f"{study}_cell_line_data.csv")
        removed_df.to_csv(file_path, index=False)
        ...
        dupe_mask = post_validation_df.duplicated(
        ...
        final_df = post_validation_df[~dupe_mask]
        ...
        print(
            f"Removed {patient_variant_dupes.shape[0]} rows with duplicated Gnomad_Notation per PATIENT_ID."
        )
