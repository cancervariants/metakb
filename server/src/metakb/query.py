...
ORDER BY s.id;
        ...
        result = self.driver.execute_query(
            ...
            condition_id=normalized_disease,
        ...
        for record in result.records:
            ...
            if statement_type == "VariantTherapeuticResponseProposition":
                ...
                strength_node = record["str"]
                method_node = record["mth"]
                method_doc_node = record["mthdoc"]
                statement = VariantTherapeuticResponseStudyStatement(
                    id=statement_node["id"],
                    description=statement_node["description"] or None,
                    direction=statement_node["direction"],
                    strength=Strength(
                        name=strength_node["name"],
                        primaryCoding=json.loads(strength_node["primaryCoding"]),
                        mappings=json.loads(strength_node["mappings"]),
                    ),
                    specifiedBy=Method(
                        id=method_node["id"],
                        name=method_node["name"],
                        method_type=node["methodType"],
                        reportedIn=Document(
                            name=method_doc_node["name"],
                            title=method_doc_node["title"],
                            pmid=method_doc_node["pmid"],
                            doi=method_doc_node["doi"],
                        ),
                    ),
