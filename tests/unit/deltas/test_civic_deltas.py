...
UPDATED_JSON = PROJECT_ROOT / "tests" / "data" / "deltas" / "updated_civic.json"
...
def diff():
    ...
    return {
        ...
        "genes": {
            "DELETE": [{"id": 3, "name": "test_remove"}],
            "INSERT": [],
            "UPDATE": [{"2778": {"aliases": {"$insert": [(1, "MIF2")]}}}],
        },
        "variants": {
            "DELETE": [],
            "INSERT": [],
            "UPDATE": [{"27": {"$delete": ["entrez_name"]}}],
        },
        "assertions": {
            "DELETE": [],
            "INSERT": [{"id": 1, "description": "description"}],
            "UPDATE": [],
        },
        "evidence": {
            "INSERT": [],
            "DELETE": [],
            "UPDATE": [{"358": {"variant_origin": "Somatic"}}],
        },
    }
...
def delta():
    ...
    return {
        "genes": {"DELETE": [], "INSERT": [], "UPDATE": []},
        "variants": {"DELETE": [], "INSERT": [], "UPDATE": []},
        "evidence": {"DELETE": [], "INSERT": [], "UPDATE": []},
        "assertions": {"DELETE": [], "INSERT": [], "UPDATE": []},
    }
...
def test_ins_del_delta(civic, diff, main_data, updated_data, delta):
    ...
    civic._ins_del_delta(delta, "assertions", "INSERT", [1], updated_data["assertions"])
...
def test_update_delta(civic, diff, delta, updated_data, main_data):
    ...
    civic._update_delta(delta, "genes", updated_data["genes"], main_data["genes"])
    assert delta["genes"]["UPDATE"] == diff["genes"]["UPDATE"]

    civic._update_delta(
        delta, "variants", updated_data["variants"], main_data["variants"]
    )
    assert delta["variants"]["UPDATE"] == diff["variants"]["UPDATE"]

    civic._update_delta(
        delta, "evidence", updated_data["evidence"], main_data["evidence"]
    )
...
def test_create_json(civic, diff):
    ...
    file_name = (
        APP_ROOT / "data" / "civic" / "delta" / f"civic_deltas" f"_{test_date}.json"
    )
