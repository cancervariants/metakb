...
UPDATED_JSON = PROJECT_ROOT / "tests" / "data" / "deltas" / "updated_moa.json"
...
def diff():
    ...
    return {
        ...
        "sources": {
            ...
            "UPDATE": [{"2": {"assertion_id": {"$insert": [(1, 3)]}}}],
        },
        "variants": {
            "DELETE": [{"id": 5, "feature_type": "test_removal"}],
            "INSERT": [],
            "UPDATE": [],
        },
    }
...
def test_ins_del_delta(moa, diff, main_data, updated_data, delta):
    ...
    moa._ins_del_delta(delta, "sources", "INSERT", [22], updated_data["sources"])
...
def test_update_delta(moa, diff, delta, updated_data, main_data):
    ...
    moa._update_delta(
        delta, "assertions", updated_data["assertions"], main_data["assertions"]
    )
    assert delta["assertions"]["UPDATE"] == diff["assertions"]["UPDATE"]

    moa._update_delta(delta, "sources", updated_data["sources"], main_data["sources"])
...
def test_create_json(moa, diff):
    ...
    file_name = APP_ROOT / "data" / "moa" / "delta" / f"moa_deltas_" f"{test_date}.json"
