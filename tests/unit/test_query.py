...
def assert_keys_for_detail_true(
    response_keys, response, is_evidence=True, tr_response=True
):
    ...
    for field in fields:
        ...
        if is_evidence:
            ...
            if field == "documents" and response["statements"][0]["id"].startswith(
                SourceName.ONCOKB
            ):
...
def assert_response_items(
    ...
    check_variation_descriptor(response_variation_descriptor, variation_descriptor)
    ...
    assert (
        response_statement["variation_descriptor"]
        == response_variation_descriptor["id"]
    )
    if therapy_descriptor:
        assert (
            response_statement["therapy_descriptor"]
            == response_therapy_descriptor["id"]
        )
    assert response_statement["disease_descriptor"] == response_disease_descriptor["id"]
    ...
    assert proposition["subject"] == response_variation_descriptor["variation_id"]
    assert proposition["object_qualifier"] == response_disease_descriptor["disease_id"]
    if therapy_descriptor:
        assert proposition["object"] == response_therapy_descriptor["therapy_id"]

    assert (
        response_variation_descriptor["gene_context"] == response_gene_descriptor["id"]
    )
...
async def test_civic_eid2997(
    ...
    # Alt Label
    s, p = await return_response(
        query_handler, statement_id, variation="egfr Leu858ARG"
    )
    check_statement(s, civic_eid2997_statement)
    check_proposition(p, civic_eid2997_proposition)
    ...
    # HGVS Expression
    s, p = await return_response(
        query_handler, statement_id, variation="NP_005219.2:p.Leu858Arg"
    )
    check_statement(s, civic_eid2997_statement)
    check_proposition(p, civic_eid2997_proposition)
    ...
    # Label
    s, p = await return_response(
        query_handler, statement_id, disease="Lung Non-small Cell Carcinoma"
    )
    check_statement(s, civic_eid2997_statement)
    check_proposition(p, civic_eid2997_proposition)
...
@pytest.mark.asyncio
async def test_civic_eid1409_statement(
    query_handler, civic_eid1409_statement, check_statement
):
    ...
    # # Alt Label
    s, p = await return_response(
        query_handler, statement_id, variation="braf val600glu"
    )
    check_statement(s, civic_eid1409_statement)

    # HGVS Expression
    s, p = await return_response(
        query_handler, statement_id, variation="NP_004324.2:p.Val600Glu"
    )
    check_statement(s, civic_eid1409_statement)
...
async def test_multiple_parameters(query_handler):
    ...
    response = await query_handler.search(
        variation=" braf v600e", gene="egfr", disease="cancer", therapy="cisplatin"
    )
    ...
    response = await query_handler.search(
        variation="NP_005219.2:p.Leu858Arg", disease="NSCLC", therapy="Afatinib"
    )
    ...
    response = await query_handler.search(
        variation=subject, disease="malignant trunk melanoma"
    )
    for p in response["propositions"]:
        if p["id"] in response["matches"]["propositions"]:
            assert p["object_qualifier"] == object_qualifier
    ...
    assert set(civic_aid6_supported_by_statements) == set(supported_by_statements)

    response = await query_handler.search(
        disease="ncit:C2926", variation="ga4gh:VA.kgjrhgf84CEndyLjKdAO0RxN-e3pJjxA"
    )
    statement_ids = list()
    for s in response["statements"]:
        if s["id"] == "civic.aid:6":
...
async def test_oncokb_braf_v600e(
    ...
    response = await query_handler.search(
        statement_id=statement_id, variation="BRAF V600E", detail=True
    )
    assert_keys_for_detail_true(response.keys(), response, tr_response=False)
    assert_response_items(
        response,
...
def test_moa_id_search(
    ...
    check_descriptor(res["disease_descriptor"], moa_chronic_myelogenous_leukemia)
...
async def test_service_meta(query_handler):
    ...
    def check_service_meta(response):
        ...
        assert service_meta_["url"] == "https://github.com/cancervariants/metakb"
