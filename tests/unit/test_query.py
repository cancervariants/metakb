...
def return_response(query_handler, statement_id, **kwargs):
    ...
    for statement in statements:
        if statement.id.__root__ == statement_id:
            s = statement
            break
...
def assert_no_match_id(response):
    ...
    assert len(response.__fields_set__) == 3
    assert len(response.warnings) > 0
...
def assert_keys_for_detail_true(response_keys, response, is_evidence=True,
    ...
    for field in fields:
        assert field in response_keys.__fields_set__
        if is_evidence:
...
def assert_response_items(response, statement, proposition,
    ...
    if therapy_descriptor:
        assert_keys_for_detail_true(response.__fields_set__, response)
    else:
        assert_keys_for_detail_true(response.__fields_set__, response,
                                    tr_response=False)
    ...
    check_statement(response_statement.dict(), statement)
...
def test_civic_eid2997(query_handler, civic_eid2997_statement,
    ...
    check_statement(s.dict(), civic_eid2997_statement)
    check_proposition(p.dict(), civic_eid2997_proposition)
...
def test_civic_eid1409_statement(query_handler, civic_eid1409_statement,
    ...
    check_statement(s.dict(), civic_eid1409_statement)
...
def test_civic_aid6(query_handler, civic_aid6_statement, check_statement):
    ...
    s, p = return_response(query_handler, statement_id,
                           variation='ga4gh:VA.kgjrhgf84CEndyLjKdAO0RxN-e3pJjxA')  # noqa: E501
    check_statement(s.dict(), civic_aid6_statement)
...
def test_civic_detail_flag_therapeutic(query_handler,
    ...
    response = query_handler.search(statement_id='civic.eid:2997',
                                    detail=False)
    assert_keys_for_detail_false(response.__fields_set__)
...
def test_civic_detail_flag_prognostic(query_handler, civic_eid26_statement,
    ...
    response = query_handler.search(statement_id='civic.eid:26',
                                    detail=True)
    assert_keys_for_detail_true(response.__fields_set__, response, tr_response=False)
...
def test_moa_id_search(query_handler, moa_aid71_statement,
    ...
    check_statement(res.statement.dict(exclude_none=True), moa_aid71_statement)

    res = query_handler.search_by_id('moa.variant:71')
    check_variation_descriptor(
        res.variation_descriptor.dict(exclude_none=True), moa_vid71)

    res = query_handler.search_by_id('moa.normalize.gene:ABL1')
    check_descriptor(res.gene_descriptor.dict(exclude_none=True), moa_abl1)

    res = query_handler.search_by_id('moa.normalize.therapy:Imatinib')
    check_descriptor(res.therapy_descriptor.dict(exclude_none=True), moa_imatinib)

    res = query_handler.search_by_id('moa.normalize.disease:oncotree%3ACML')
    check_descriptor(res.disease_descriptor.dict(exclude_none=True),
    ...
    check_document(res.document.dict(exclude_none=True), pmid_11423618)

    res = query_handler.search_by_id(' method:4 ')
    check_method(res.method.dict(exclude_none=True), method4)
...
def test_service_meta(query_handler):
    ...
    def check_service_meta(response):
        ...
        assert "service_meta_" in response.__fields_set__
