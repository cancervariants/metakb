...
async def test_paginate(query_handler: QueryHandler, normalizers):
    ...
    braf_va_id = "ga4gh:VA.Otc5ovrw906Ack087o1fhegB4jDRqCAe"
    full_response = await query_handler.batch_search_studies([braf_va_id])
    paged_response = await query_handler.batch_search_studies([braf_va_id], start=1)
    ...
    huge_page_response = await query_handler.batch_search_studies(
    ...
    # get last item
    last_response = await query_handler.batch_search_studies(
        [braf_va_id], start=len(full_response.study_ids) - 1
    )
    assert len(last_response.study_ids) == 1
    assert last_response.study_ids[0] == full_response.study_ids[-1]

    # test limit
    min_response = await query_handler.batch_search_studies([braf_va_id], limit=1)
    assert min_response.study_ids[0] == full_response.study_ids[0]

    # test limit and start
    other_min_response = await query_handler.batch_search_studies(
        [braf_va_id], start=1, limit=1
    )
    assert other_min_response.study_ids[0] == full_response.study_ids[1]

    # test limit of 0
    empty_response = await query_handler.batch_search_studies([braf_va_id], limit=0)
    ...
    with pytest.raises(ValueError, match="Can't start from an index of less than 0."):
        await query_handler.batch_search_studies([braf_va_id], start=-1)
    with pytest.raises(
        ValueError, match="Can't limit results to less than a negative number."
    ):
        await query_handler.batch_search_studies([braf_va_id], limit=-1)

    # test default limit
    limited_query_handler = QueryHandler(normalizers=normalizers, default_page_limit=1)
    default_limited_response = await limited_query_handler.batch_search_studies(
        [braf_va_id]
    )
    ...
    # test overrideable
    less_limited_response = await limited_query_handler.batch_search_studies(
        [braf_va_id], limit=2
    )
    ...
    skipped_limited_response = await limited_query_handler.batch_search_studies(
        [braf_va_id], start=1
    )
