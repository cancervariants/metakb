...
class Graph:
    ...
    If not given arguments, attempts to connect via

    1. Parameters from AWS Secrets Manager, if in a production environment (designated
       by ``METAKB_NORM_EB_PROD`` environment variable)
    2. Environment variables ``METAKB_DB_URL``, ``METAKB_DB_USERNAME``, and
       ``METAKB_DB_PASSWORD``. Must set all of them for any to be used.
    ...
    def __new__(
        ...
        """Get new reference to singleton instance.

        :param uri: connection URI, formatted a la ``bolt://<host>:<port #>``
        :param credentials: tuple containing username and password
        :param check_constraints: check for, and create if necessary, graph constraints
        :return: shared graph instance
        """
        if cls._instance is None:
            uri, credentials = _get_credentials(uri, credentials)
            cls._instance = super().__new__(cls)
            cls._instance.driver = GraphDatabase.driver(uri, auth=credentials)
            if check_constraints:
                with cls._instance.driver.session() as session:
                    cls._create_constraints(session)
        return cls._instance

    @classmethod
    def _create_constraints(cls, session: Session) -> None:
        """Create unique property constraints for nodes

        :param session: Session instance for executing constraint queries
        """
        queries = [
    ...
    def close(self) -> None:
        """Close graph driver, if it exists."""
        if self.driver:
            self.driver.close()

    def __del__(self) -> None:
        """Perform graph destruction. Attempt to close connection."""
        try:
            self.close()
        except Exception as e:
            _logger.error("Encounted %s while attempting to clean up Graph object", e)
...
def _get_secret() -> str:
    ...
    :return: code formatted as string
    """
