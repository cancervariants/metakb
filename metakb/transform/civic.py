...
class CIViCTransform(Transform):
    ...
    def _mp_to_variant_mapping(molecular_profiles: List[Dict]) -> Dict:
        ...
        logger.debug(
            f"{len(not_supported_mps)} Molecular Profiles not supported: "
            f"{not_supported_mps}"
        )
        return mapping
    ...
    async def transform(self):
        ...
        vids |= {
            mp_id_to_v_id_mapping[a["molecular_profile_id"]]
            for a in assertions
            if a["molecular_profile_id"]
        }
        ...
        for r in records:
            ...
            if record_type == "PREDICTIVE":
                if len(r["therapies"]) != 1:
                    ...
                    therapy_descriptor = self._add_therapy_descriptor(therapy_id, r)
            ...
            if is_evidence:
                ...
                if r["amp_level"] and not r["acmg_codes"]:
                    method = f"method:" f"{schemas.MethodID.CIVIC_AID_AMP_ASCO_CAP}"
        ...
        if assertion["amp_level"]:
            if assertion["amp_level"] == "Not Applicable":
                ...
                if match:
                    ...
                    evidence_level = f"amp_asco_cap_2017_level:" f"{tier}{level}"
                else:
                    raise Exception(f"{amp_level} not supported with regex")
        return evidence_level

    def _get_proposition_type(
        self, evidence_type, is_evidence=True
    ) -> Optional[schemas.PropositionType]:
        ...
        if evidence_type in schemas.PropositionType.__members__.keys():
            ...
            raise KeyError(
                f"Proposition Type {evidence_type} not found in "
    ...
    def _get_predicate(self, proposition_type, clin_sig) -> Optional[schemas.Predicate]:
        ...
        if proposition_type == schemas.PropositionType.PREDICTIVE:
            ...
            logger.warning(
                f"CIViC proposition type: {proposition_type} "
    ...
    async def _add_variation_descriptors(self, variants: List, vids: Set) -> None:
        ...
        for variant in variants:
            ...
            if not variation_descriptor:
                logger.warning(
                    "Variation Normalizer unable to normalize "
            ...
            variation_descriptor = VariationDescriptor(
                ...
                alternate_labels=[
                    v_alias
                    for v_alias in variant["variant_aliases"]
            ...
            if xref == "clinvar_entries":
                for clinvar_entry in v["clinvar_entries"]:
                    if clinvar_entry and clinvar_entry not in ["N/A", "NONE FOUND"]:
                        xrefs.append(
                            f"{schemas.XrefSystem.CLINVAR.value}:" f"{clinvar_entry}"
                        )
            elif xref == "allele_registry_id" and v["allele_registry_id"]:
                xrefs.append(
                    f"{schemas.XrefSystem.CLINGEN.value}:" f"{v['allele_registry_id']}"
                )
            elif xref == "variant_aliases":
                dbsnp_xrefs = [
                    item for item in v["variant_aliases"] if item.startswith("RS")
                ]
                for dbsnp_xref in dbsnp_xrefs:
                    xrefs.append(
                        f"{schemas.XrefSystem.DB_SNP.value}:"
        ...
        for hgvs_expr in variant["hgvs_expressions"]:
            ...
            if hgvs_expr != "N/A":
                hgvs_expressions.append(
                    Expression(syntax=syntax, value=hgvs_expr).dict(exclude_none=True)
                )
    ...
    def _add_gene_descriptors(self, genes) -> None:
        ...
        for gene in genes:
            ...
            if normalized_gene_id:
                ...
                logger.warning(
                    f"Gene Normalizer unable to normalize {gene_id}"
                    f"using queries: {queries}"
                )

    def _add_disease_descriptor(
        self, disease_id, record
    ...
    def _get_disease_descriptors(self, disease) -> Optional[ValueObjectDescriptor]:
    ...
    def _add_therapy_descriptor(
        self, therapy_id, record
    ) -> Optional[ValueObjectDescriptor]:
        ...
        """
        therapy_descriptor = self.valid_ids["therapy_descriptors"].get(therapy_id)
        if therapy_descriptor:
            return therapy_descriptor
        else:
            therapy_descriptor = None
            if therapy_id not in self.invalid_ids["therapy_descriptors"]:
                therapy_descriptor = self._get_therapy_descriptor(
                    record["therapies"][0]
                )
                if therapy_descriptor:
    ...
    def _get_therapy_descriptor(self, drug) -> Optional[ValueObjectDescriptor]:
        ...
        if not normalized_therapy_id:
            logger.warning(
                f"Therapy Normalizer unable to normalize: "
    ...
    def _add_methods(self) -> None:
        ...
        self.methods = [
            schemas.Method(
                id=f"method:" f"{schemas.MethodID.CIVIC_EID_SOP}",
            ...
            schemas.Method(
                id=f"method:" f"{schemas.MethodID.CIVIC_AID_AMP_ASCO_CAP.value}",
                ...
                version=schemas.Date(year=2017, month=1).dict(exclude_none=True),
                authors="Li MM, Datto M, Duncavage EJ, et al.",
            ).dict(exclude_none=True),
            schemas.Method(
                id=f"method:" f"{schemas.MethodID.CIVIC_AID_ACMG.value}",
                label="Standards and guidelines for the "
                ...
                version=schemas.Date(year=2015, month=5).dict(exclude_none=True),
    ...
    def _get_aid_document(self, assertion: Dict) -> List[schemas.Document]:
        ...
        if label and version:
            ...
            documents = list()
            documents.append(
                schemas.Document(id=db_id, document_id=doc_id, label=doc_label).dict(
                    exclude_none=True
                )
            )
