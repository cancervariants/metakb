...
class VICCNormalizers:
    ...
    def normalize_gene(
        ...
        :param List[str] queries: Gene queries to normalize
        ...
        for query_str in queries:
            ...
            try:
                ...
                if gene_norm_resp.match_type > highest_match:
                    ...
                    normalized_gene_id = gene_norm_resp.gene_descriptor.gene
    ...
    def normalize_disease(
        ...
        :param List[str] queries: Disease queries to normalize
        ...
        for query in queries:
            ...
            try:
                ...
                if disease_norm_resp.match_type > highest_match:
                    ...
                    normalized_disease_id = disease_norm_resp.disease_descriptor.disease
                    if highest_match == 100:
                        break
    ...
    def normalize_therapy(
        ...
        :param List[str] queries: Therapy queries to normalize
        :return: The highest matched therapy's normalized response and ID
        """
        highest_match = 0
        normalized_therapeutic_id = None
        ...
        for query in queries:
            ...
            try:
                ...
                logger.warning(f"Therapy Normalizer raised an exception using query "
                               f"{query}: {e}")
            else:
                if therapy_norm_resp.match_type > highest_match:
                    highest_match = therapy_norm_resp.match_type
                    normalized_therapeutic_id = therapy_norm_resp.therapeutic_descriptor.therapeutic  # noqa: E501
                    if highest_match == 100:
                        break
        return therapy_norm_resp, normalized_therapeutic_id
    ...
    def get_regulatory_approval_extension(
        ...
        tn_resp_exts = therapy_norm_resp.dict().get("therapeutic_descriptor", {}).get("extensions") or []  # noqa: E501
