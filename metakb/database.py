...
def _create_keys_string(entity, keys) -> str:
    ...
    """
    nonnull_keys = [f"{key}:${key}" for key in keys if entity.get(key)]
...
class Graph:
    ...
    def _create_constraints(tx):
        ...
        for label in [
            ...
            try:
                ...
                logging.error(
                    f"Failed to generate ID property " f"constraint for {label}."
                )
                raise exception
    ...
    def add_transformed_data(self, data: Dict):
        ...
        with self.driver.session() as session:
            ...
            for method in data.get("methods", []):
                try:
                    session.write_transaction(self._add_method, method, added_ids)
            ...
            for descriptor in [
                ...
                for d in data.get(descriptor, []):
                    try:
                        session.write_transaction(self._add_descriptor, d, added_ids)
                    except ConstraintError:
            ...
            for var_descr in data.get("variation_descriptors", []):
                try:
                    session.write_transaction(
                        self._add_variation_descriptor, var_descr, added_ids
                    )
                except ConstraintError:
            ...
            for proposition in data.get("propositions", []):
                try:
                    session.write_transaction(self._add_proposition, proposition)
                except ConstraintError:
            ...
            for s in data.get("statements", []):
                loaded_count += 1
                try:
                    session.write_transaction(self._add_statement, s, added_ids)
                except ConstraintError:
    ...
    def _add_method(tx, method: Dict, added_ids: Set[str]):
        ...
        if method["id"] in added_ids:
            try:
                ...
                logging.error(
                    f"Failed to add Method object\nQuery: "
                    f"{query}\nAssertionMethod: {method}"
                )
                raise exception

    @staticmethod
    def _update_fmt_key_extensions(
        fmt_keys: str, extensions: List[Dict], obj: Dict
    ) -> str:
    ...
    def _add_descriptor(self, tx, descriptor: Dict, added_ids: Set[str]):
        ...
        descr_keys = _create_keys_string(
            descriptor, ("id", "label", "description", "xrefs", "alternate_labels")
        )
        ...
        if descr_type in {"TherapyDescriptor", "GeneDescriptor", "DiseaseDescriptor"}:
            descr_keys = self._update_fmt_key_extensions(
                descr_keys, extensions, descriptor
            )
        ...
        try:
            ...
            logging.error(
                f"Failed to add Descriptor object\nQuery: {query}\n"
                f"Descriptor: {descriptor}"
            )
            raise exception

    @staticmethod
    def _add_variation_descriptor(tx, descriptor_in: Dict, added_ids: Set[str]):
        ...
        try:
            ...
            logging.error(
                f"Failed to add Variant Descriptor object\nQuery: "
    ...
    def _add_proposition(tx, proposition: Dict):
        ...
        """
        formatted_keys = _create_keys_string(proposition, ("id", "predicate", "type"))
        ...
        try:
            ...
            logging.error(
                f"Failed to add Proposition object\n"
    ...
    def _add_document(tx, document: Dict):
        ...
        try:
            ...
            logging.error(
                f"Failed to read Document object\n"
                f"Query: {query}\nDocument: "
                f"{document}"
            )
            raise exception
        ...
        if not result.single():
            formatted_keys = _create_keys_string(
                document, ("id", "label", "document_id", "xrefs", "description")
            )
            query = f"""
            ...
            try:
                ...
                logging.error(
                    f"Failed to add Document object\n"
                    f"Query: {query}\nDocument: "
                    f"{document}"
                )
                raise exception
    ...
    def _get_ids_from_statement(self, statement, added_ids):
        ...
        """
        for node_id in [
            statement.get("therapy_descriptor"),
    ...
    def _add_statement(self, tx, statement: Dict, added_ids: Set[str]):
        ...
        formatted_keys = _create_keys_string(
            statement,
            ("id", "description", "direction", "variation_origin", "evidence_level"),
        )

        extensions = statement.get("extensions", [])
        formatted_keys = self._update_fmt_key_extensions(
            formatted_keys, extensions, statement
        )
        ...
        if td:
            therapy_descriptor = (
                f"MERGE (ther:TherapyDescriptor {{id:$therapy_descriptor}})"  # noqa: F541
            )
        ...
        try:
            ...
            logging.error(
                f"Failed to add Evidence object\n"
                f"Query: {query}\nEvidence: {statement}"
            )
            raise exception
