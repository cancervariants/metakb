...
import boto3
import gene.cli as GeneCLI  # noqa: N812
...
from metakb.harvesters import CIViCHarvester, Harvester, MOAHarvester, OncoKBHarvester
from metakb.schemas import SourceName
from metakb.transform import CIViCTransform, MOATransform, OncoKBTransform, Transform
...
class CLI:
    ...
    @click.option(
        ...
        required=False,
        help=(
            "If set to `True`, civicpy.update_cache will first download the remote "
    ...
    @click.option(
        ...
        type=click.Path(exists=True, dir_okay=False, readable=True, path_type=Path),
        help=(
            "Path to CSV file containing header row with `hugo_symbol` and "
            ...
            for src in sorted({v.value for v in SourceName.__members__.values()}):
                ...
                try:
                    ...
                    raise FileNotFoundError(
                        f"No valid transform file found " f"matching pattern: {pattern}"
                    )
    ...
    def _retrieve_s3_cdms(self) -> str:
        ...
        if newest_version is None:
            raise FileNotFoundError(
                "Unable to locate files matching expected "
