...
from metakb.schemas.api import SearchStudiesService, SearchIdService
...
search_description = ("Return studies and associated to the queried concepts.")
...
# ):
#     """Search endpoint"""
#     resp = await query.search(variation, disease, therapy, gene, study_id, detail)
#     return resp
...
search_study_response = "A response to a validly-formed query."
search_study_descr = ("Return nested studies associated to the queried concepts.")


@app.get('/api/v2/search/studies',
         summary=search_studies_summary,
         response_description=search_study_response,
         response_model=SearchStudiesService,
         description=search_study_descr,
         response_model_exclude_none=True)
...
id_query_desc = ("Given Meta-KB study_id, proposition_id, descriptor_id,"
...
async def search_by_id(
    id: str = Path(..., description=id_description)
) -> SearchIdService:
